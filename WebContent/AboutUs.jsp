<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<!DOCTYPE html>
<html lang="en">

<head>

<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport"
	content="width=device-width, initial-scale=1, shrink-to-fit=no">
<meta name="description" content="">
<meta name="author" content="">

<title>Building Management and Compliance</title>

<!-- Custom fonts for this template-->
<link href="vendor/fontawesome-free/css/all.min.css" rel="stylesheet"
	type="text/css">
<link
	href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
	rel="stylesheet">

<!-- Custom styles for this template-->
<link href="css/bootstrap.min.css" rel="stylesheet">
<link href="css/sb-admin-2.min.css" rel="stylesheet">

<!-- Custom styles for this page -->
<link href="vendor/datatables/dataTables.bootstrap4.min.css"
	rel="stylesheet">


</head>

<body id="page-top">

	<!-- Include Header.jsp -->
	<%@ include file="Header.jsp"%>

	<!-- Begin Page Content -->
	<div class="container-fluid">

		<!-- About Us Text -->
		<div class="text-center">


			<h1 style="font-size: 60px; font-weight: bold"
				class="lead text-gray-800 mb-20">About Us</h1>
		</div>

	</div>



	<!-- /.container-fluid -->

	<!-- Begin Page Content -->
	<div class="container-fluid">

		<!-- Content Row -->
		<div class="row">
			<div class="col-xl-7 col-lg-7">

				<!-- About Us -->
				<div class="card shadow mb-4">
					<div class="card-header py-3">
						<h6 class="m-0 font-weight-bold text-primary">About Us</h6>
					</div>
					<div class="card-body">
						<div>
							<br>
							<p class="h2 mb-1 text-gray-600" style="text-indent: 40px">Building
								Management & Compliance is committed to making Building
								Management & Compliance easy. This system provides your
								organisation with tools needed to ensure that you can
								demonstrate compliance with all legislative obligations relevant
								to buildings.</p>

						</div>
						<br> <br> <br> <br>
					</div>
				</div>

			</div>

			<!-- Image Skyscraper -->
			<div class="col-xl-5 col-lg-5">
				<div class="card shadow mb-4">

					<!-- Our Buildings -->
					<div class="card-header py-3">
						<h6 class="m-0 font-weight-bold text-primary">Our Buildings</h6>
					</div>
					<!-- Card Body -->
					<div class="card-body">
						<div>
							<img src="images/building_skyscraper.jpg" alt="skyscrapper"
								height=310 width=100%>
						</div>
					</div>
				</div>
			</div>
		</div>

	</div>
	<!-- /.container-fluid -->

	<br>



	<!-- The Professionals Section -->

	<div class="container-fluid">


		<section class="page-section" id="services">
			<div class="container">
				<div class="text-center">
					<h1 style="font-size: 30px" class="h3 mb-1 text-gray-800">The
						Professionals</h1>
				</div>
				<hr class="divider my-4">
				<div class="row">
					<div class="col-lg-3 col-md-6 text-center">
						<div class="card mb-4 py-3 border-bottom-info">
							<div class="card-body">
								<i class="fas fa-4x fa-gem text-primary mb-4"></i>
								<h3 class="h2 mb-1 text-gray-800">Natasha Baldwin</h3>
								<p class="text-muted mb-0">CEO - Building Management &
									Compliance</p>
							</div>
						</div>
					</div>
					<div class="col-lg-3 col-md-6 text-center">
						<div class="card mb-4 py-3 border-bottom-info">
							<div class="card-body">
								<i class="fas fa-4x fa-laptop-code text-primary mb-4"></i>
								<h3 class="h2 mb-1 text-gray-800">Kristel Villanueva</h3>
								<br>
								<p class="text-muted mb-0">Lead Developer</p>
								<br>
							</div>
						</div>
					</div>
					<div class="col-lg-3 col-md-6 text-center">
						<div class="card mb-4 py-3 border-bottom-info">
							<div class="card-body">
								<i class="fas fa-4x fa-globe text-primary mb-4"></i>
								<h3 class="h2 mb-1 text-gray-800">Angela Kuznetsova</h3>
								<br>
								<p class="text-muted mb-0">Junior Developer</p>
								<br>
							</div>
						</div>
					</div>
					<div class="col-lg-3 col-md-6 text-center">
						<div class="card mb-4 py-3 border-bottom-info">
							<div class="card-body">
								<i class="fas fa-4x fa-heart text-primary mb-4"></i>
								<h3 class="h2 mb-1 text-gray-800">Dexter Balsamo</h3>
								<br>
								<p class="text-muted mb-0">Junior Developer</p>
								<br>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section>

	</div>
	<!-- End of The Professionals Section -->
	<!-- End of Main Content -->

	<!-- Include Footer.jsp -->
	<%@ include file="Footer.jsp"%>

</body>

</html>
